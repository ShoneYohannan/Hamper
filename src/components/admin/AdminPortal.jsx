...
import { products as defaultCatalogProducts, reserveProducts as defaultCatalogReserve } from '../../data/products';
...
export default function AdminPortal() {
  ...
  // Identify any default hampers that have been deleted
  const currentProductIds = new Set(products.map(p => String(p.id)));
  const currentReserveIds = new Set(reserveProducts.map(p => String(p.id)));

  const deletedDefaultProducts = defaultCatalogProducts.filter(p => !currentProductIds.has(String(p.id)));
  const deletedDefaultReserve = defaultCatalogReserve.filter(p => !currentReserveIds.has(String(p.id)));
  const allDeletedDefaults = [...deletedDefaultProducts, ...deletedDefaultReserve];
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar">
          ...
          {activeTab === 'products' && (
            <div className="space-y-6 max-w-6xl mx-auto">
              ...
              {/* Deleted Hampers Quick Recovery Banner */}
              {allDeletedDefaults.length > 0 && (
                <div className="bg-amber-950/25 border border-amber-500/40 p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-fade-in">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-white text-xs font-semibold uppercase tracking-wider">
                        {allDeletedDefaults.length} Deleted Default Hamper{allDeletedDefaults.length > 1 ? 's' : ''} Available for Recovery
                      </h4>
                      <p className="text-[11px] text-neutral-300 mt-0.5">
                        Did you accidentally delete a hamper? You can restore it with 1 click.
                      </p>
                    </div>
                  </div>

                  <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                    {allDeletedDefaults.map(dh => (
                      <button
                        key={dh.id}
                        onClick={() => {
                          restoreDefaultHamper(dh.id);
                          alert(`Restored "${dh.name}" back to the catalog!`);
                        }}
                        className="px-3 py-1.5 rounded-lg text-[11px] font-semibold bg-white/10 hover:bg-white/20 text-white border border-white/15 flex items-center gap-1.5 transition-all shadow"
                        title={`Restore ${dh.name}`}
                      >
                        <Plus className="w-3 h-3 text-[#D4AF37]" />
                        <span>Restore "{dh.name.slice(0, 20)}..."</span>
                      </button>
                    ))}
                    ...
                    <button
                      onClick={() => {
                        restoreAllHampers();
                        alert('All default hampers restored successfully!');
                      }}
                      className="px-3.5 py-1.5 rounded-lg text-[11px] font-semibold bg-[#D4AF37] text-black hover:brightness-110 flex items-center gap-1.5 transition-all shadow"
                    >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-serif text-lg text-white font-normal">
                        Restore Deleted Default Hampers
                      </h4>
                      <p className="text-xs text-neutral-400">
                        {allDeletedDefaults.length > 0 
                          ? `${allDeletedDefaults.length} default hamper(s) currently missing from the catalog.` 
                          : 'All default atelier hampers are currently active in your catalog.'}
                      </p>
                    </div>
                  </div>
                  ...
                  {allDeletedDefaults.length > 0 && (
                    <button
                      ...
                      className="px-4 py-2 rounded-full text-xs font-semibold bg-[#D4AF37] text-black hover:brightness-110 flex items-center gap-1.5 transition-all shadow"
                    >
                ...
                {allDeletedDefaults.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 pt-2">
                    {allDeletedDefaults.map(dh => (
                      <div 
                        key={dh.id}
                        className="bg-white/5 border border-white/10 p-3 rounded-2xl flex items-center justify-between gap-2"
                      >
                        <div className="truncate">
                          <p className="text-xs font-medium text-white truncate">{dh.name}</p>
                          <p className="text-[10px] text-[#D4AF37]">{dh.price} AED · {dh.badge || 'Default'}</p>
                        </div>
                        <button
                          onClick={() => {
                            restoreDefaultHamper(dh.id);
                            alert(`Restored "${dh.name}"!`);
                          }}
                          className="px-2.5 py-1 rounded-lg text-[10px] font-semibold bg-[#D4AF37]/20 text-[#F3E5AB] hover:bg-[#D4AF37] hover:text-black transition-all shrink-0"
                        >
                          Restore
                        </button>
                      </div>
